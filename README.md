...
[![Version](https://img.shields.io/badge/version-0.20.4-blue)](addon/CHANGELOG.md)
...
> ✅ **Stato: MVP operativo (v0.20.4).** Calendario, giocatori, presenze, formazioni, piani partita AI ed esportazioni Google sono disponibili; la roadmap residua è descritta in [`SPECS.md`](SPECS.md).
...
- **Piani partita** — crea le formazioni manualmente sul campo virtuale oppure genera rotazioni AI, con minutaggio bilanciato, ruoli e fallback locale.
...
Il frontend in sviluppo proxya le richieste `/api` verso il backend (vedi `addon/frontend/vite.config.ts`). In Docker Compose il proxy usa automaticamente il nome del servizio `backend`; nell'avvio nativo usa `localhost:3002`. Puoi sovrascrivere il target con `VITE_API_PROXY_TARGET`.
