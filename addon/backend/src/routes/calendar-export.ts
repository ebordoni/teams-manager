...
import { hasGoogleCalendarAccess, getAuthorizedClient } from "../services/google/auth";
...
const router = Router();
const ExportSchema = z.object({ eventIds: z.array(z.number().int().positive()).min(1) });
...
router.post("/export", async (req: Request, res: Response) => {
  ...
  if (!hasGoogleCalendarAccess()) {
    return void res.status(403).json({ error: "Ricollega Google per autorizzare l'accesso a Calendar" });
  }
  ...
  const eventTypes = new Map(
    (db.prepare("SELECT * FROM event_types").all() as unknown as EventTypeDefRow[])
      .map((row) => {
  ...
  try {
    for (const eventId of new Set(parse.data.eventIds)) {
      ...
      const mapping = db.prepare("SELECT google_event_id FROM google_calendar_exports WHERE event_id = ?")
        .get(eventId) as { google_event_id: string } | undefined;
      const googleEventId = await upsertCalendarEvent(auth, calendarId, event, eventTypes.get(event.type), mapping?.google_event_id);
      db.prepare(
    ...
    res.status(502).json({ error: "Impossibile esportare gli eventi su Google Calendar" });
  }
});
