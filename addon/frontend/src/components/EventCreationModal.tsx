...
export default function EventCreationModal({
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      notifications.show({
        color: "red",
        message: message ?? "Impossibile creare l'evento",
      });
    } finally {
      setSaving(false);
    }
        ...
        <Stack gap="lg">
            ...
            >
              {eventTypes.map((type) => (
                <Tabs.Tab
                  key={type.key}
                  value={type.key}
                  leftSection={<EventTypeIcon name={type.icon} size={16} />}
                  style={{ flex: "0 0 auto" }}
                >
                  {type.label}
                </Tabs.Tab>
          ...
          <Stack gap="sm">
            ...
            <div>
              ...
              <Button.Group>
                <Button
                  type="button"
                  variant={!form.repeatWeekly ? "filled" : "default"}
                  onClick={() =>
                    setForm((current) => ({
                      ...current,
                      repeatWeekly: false,
                ...
                <Button
                  ...
                  variant={form.repeatWeekly ? "filled" : "default"}
                  onClick={() =>
                    setForm((current) => ({ ...current, repeatWeekly: true }))
                  }
                >
                  Ricorrente
                </Button>
          ...
          <Stack gap="sm">
            ...
            <div>
              ...
              <Button.Group>
                <Button
                  type="button"
                  variant={form.venue === "home" ? "filled" : "default"}
                  onClick={() =>
                    setForm((current) => ({ ...current, venue: "home" }))
                  }
                >
                  In casa
                </Button>
                <Button
                  type="button"
                  variant={form.venue === "away" ? "filled" : "default"}
                  onClick={() =>
                    setForm((current) => ({ ...current, venue: "away" }))
                  }
                >
                  Trasferta
                </Button>
                <Button
                  type="button"
                  variant={form.venue === "neutral" ? "filled" : "default"}
                  onClick={() =>
                    setForm((current) => ({ ...current, venue: "neutral" }))
                  }
                >
                  Neutro
                </Button>
          ...
          <Group justify="flex-end" wrap="wrap-reverse">
            ...
            <Button type="submit" loading={saving}>
              {form.repeatWeekly ? "Crea appuntamenti" : "Crea evento"}
            </Button>
          </Group>
        </Stack>
      </form>
