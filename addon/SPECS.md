...
> Stato attuale: **MVP operativo (v0.11.0)** — le funzionalità core, le formazioni 7vs7, le comunicazioni contestuali, i controlli di qualità e il catalogo icone sono completati; restano le estensioni di roadmap.
...
| F13 | **Validazione stato evento**     | Note obbligatorie quando un evento è creato/modificato con stato "modificato" o "annullato"; badge di stato in calendario e dettaglio. | DONE  |
| F14 | **UI responsive (Mantine)**      | Interfaccia basata su Mantine UI 9 (AppShell, form, tabelle), nav a hamburger su mobile.                                               | DONE  |
| F15 | **Tipi di evento configurabili** | Gestione (crea/modifica/elimina) dei tipi evento dalle Impostazioni, con flag "ha avversario" e icona scelta da catalogo Tabler.       | DONE  |
...
GET    /api/events/:id/attendance  -- un record per convocato; `recorded: false`
                                   -- quando lo stato è solo il default proposto
...
```
GET    /api/google/status           -- { configured, connected, calendarConnected }
...
POST   /api/google/calendar/export  -- body: { eventIds: number[] } — export unidirezionale
...
GET    /api/settings                 -- { googleTemplateDocId, googleCalendarId }
PUT    /api/settings                 -- body: { googleTemplateDocId, googleCalendarId }
                                     -- (il template accetta anche l'URL completo)
```

Convenzione degli errori per gli endpoint Google: `400` richiesta non valida, `403` serve
(ri)collegare l'account (compreso il caso `invalid_grant`, in cui il token salvato viene rimosso),
`502` errore lato Google.
